...
Java における Enum とは何か、から基本的な使い方を見てきた。

Enum はクラスと同じように使うことができるが、インスタンスを生成できず、各 enum 値は実行されるプログラム中にただ 1 つ存在することが保証されたインスタンスである。

どのため、Enum の利用価値は、定数として、アプリの中では設定値して有効となる。
